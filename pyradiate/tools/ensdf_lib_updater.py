...
def download_ensdf_archive(archive_date, archive_file=ensdf_file, chunk_size=512 * 1024):
    ...
    if archive_file == ensdf_file:
        archive_config = {"date": archive_info["Last-Modified"], "size": archive_n_bytes}
...
def main():
    ...
    if latest_ensdf is None:
        logger.warning("Latest ENSDF archive date could not be read.")
